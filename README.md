...
A comprehensive control framework for organizational AI security governance, covering 10 domains with 53 sub-controls mapped to 10 normative reference frameworks. Targets mid-market organizations (50-500 employees) deploying generative, predictive, and agentic AI.
...
The JSON spec at `spec/aiss-v1.2.json` is the canonical source of truth. The markdown files are deterministically regenerated from it by `scripts/export-aiss-spec.ts`.
...
1. **Self-assessment** — Walk through the 56 sub-controls, score each at a maturity level (0-4), document evidence. Conformance baseline = all applicable SHALL sub-controls at ML2+.
